...
function preventFall(e) { 
  ...
  //renderChatBox(`${status.info}b0: ${b[0]}, b1: ${b[1]}, e.y: ${e.position.y}`);
...
function strafeMovement(entity) {
  ...
  bot.pvp.followRange = (entityCloseToWebBlock || botCloseToWebBlock || preventFall(entity)) ? 1 : 5;
...
function startClient() {
  ...
  bot.on('death', async() => {
    ...
    setTimeout( async () => {
      ...
      /*while (true) {
        for (const coordinate of zoneCoordinates) {
          const xValue = coordinate[0];
          const zValue = coordinate[1];
          const targetX = bot.entity.position.x;
          const targetZ = bot.entity.position.z;

          if (targetX === xValue && targetZ === zValue) {
            renderChatBox(`${status.ok}Zone entered`);
            break;
          } else {
